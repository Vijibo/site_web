...
function applyNightModeSettings() {
    ...
    document.querySelectorAll('#menu a, h1, h2, p, h4, li, h5, #nom1, #prenom1, #email1, #message1, #file1').forEach(el => {
        el.style.color = newColor;
    });

    document.querySelectorAll('#menu a').forEach(item => {
        item.addEventListener('mouseover', () => (item.style.color = 'gray'));
        item.addEventListener('mouseout', () => (item.style.color = newColor));
    });
}

function changeBackground() {
    currentBackground = (currentBackground + 1) % backgrounds.length;
    currentcolor_police = (currentcolor_police + 1) % couleur_police.length;
